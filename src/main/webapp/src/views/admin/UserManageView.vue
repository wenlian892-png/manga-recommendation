  ...
  <div class="user-manage-view">
    ...
    <el-table v-else :data="list" border style="width: 100%">
      ...
      <el-table-column label="操作" width="150">
        <template #default="{ row }">
          <el-button v-if="row.status === 1" size="small" type="danger" @click="handleBan(row.id)">封禁</el-button>
...
import { ADMIN_PAGE_SIZES } from '@/utils/constants'
...
async function handleUnban(row) {
  try {
    ...
    await request.post(`/admin/user/unban/${row.id}`)
